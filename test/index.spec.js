...
const history2state = history => _.omit(history, historyFields);
...
function runTests(label, { undoableConfig = {}, initialStoreState, testConfig } = {}) {
  describe(`Undoable: ${label}`, () => {
    ...
    before('setup mock reducers and states', () => {
      ...
      console.log('mock initial', mockInitialState);
    ...
    describe('Replace reducers on the fly', () => {
      const tenfoldReducer = (state = { value: 10 }, action = {}) => {
        switch (action.type) {
          case 'INCREMENT':
      ...
      it('should use replaced reducer for new actions', () => {
        ...
        let expectedResult = tenfoldReducer(_.omit(store.getState(), historyFields), {
          type: 'INCREMENT'
        });
        store.dispatch({ type: 'INCREMENT' });
        expect(_.omit(store.getState(), historyFields)).to.eql(expectedResult);

        // swap back for other tests
        store.replaceReducer(mockUndoableReducer);

        // Increment and check result again
        expectedResult = countReducer(_.omit(store.getState(), historyFields), {
          type: 'INCREMENT'
        });
        store.dispatch({ type: 'INCREMENT' });
        expect(_.omit(store.getState(), historyFields)).to.eql(expectedResult);
    ...
    describe('Actions', () => {
      it('should not record unwanted actions', () => {
        ...
        if (testConfig && testConfig.includeActions) {
          ...
          expected = {
            ...expected,
            ...history2state(notFilteredReducer(expected, excludedAction))
          };
    ...
    describe('Undo', () => {
      ...
      it("should change present state to last element of 'past'", () => {
        if (undoableConfig && undoableConfig.limit >= 0) {
          expect(history2state(undoState)).to.eql(
      ...
      it("should add a new element to 'future' from last state", () => {
        if (undoableConfig && undoableConfig.limit >= 0) {
          expect(undoState.future[0]).to.eql(history2state(incrementedState));
        }
      });
    ...
    describe('Redo', () => {
      ...
      it("should change present state to first element of 'future'", () => {
        if (undoableConfig && undoableConfig.limit >= 0) {
          expect(history2state(redoState)).to.eql(undoState.future[0]);
        }
      });

      it("should add a new element to 'past' from last state", () => {
        if (undoableConfig && undoableConfig.limit >= 0) {
          expect(redoState.past[redoState.past.length - 1]).to.eql(history2state(undoState));
        }
      });
    ...
    describe('JumpToPast', () => {
      ...
      it('should change present to a given value from past', () => {
        ...
        if (pastState !== undefined) {
          expect(_.omit(jumpToPastState, historyFields)).to.eql(pastState);
        }
      });
    ...
    describe('Clear History', () => {
      ...
      before('perform a clearHistory action', () => {
        ...
        const actionType = clearHistoryType &&
    ...
    describe('running getSlices', () => {
      if (testConfig && testConfig.checkSlices) {
        ...
        it('should keep same initial state on ignored action', () => {
          expect(history2state(secondState.normalState)).to.eql(initialState.normalState);
          expect(secondState.slice1).to.equal(initialState.slice1);
        });
        it('should increment normally', () => {
          expect(history2state(thirdState.normalState).value).to.eql(
            initialState.normalState.value + 1
          );
          expect(thirdState.slice1).to.equal(initialState.slice1);
        });
        it('should undo normally', () => {
          expect(history2state(secondState.normalState)).to.eql(
            history2state(secondState.normalState)
          );
          expect(fourthState.slice1).to.equal(initialState.slice1);
        });
        it('should redo normally', () => {
          expect(history2state(fifthState.normalState)).to.eql(
            history2state(thirdState.normalState)
          );
          expect(fifthState.slice1).to.equal(initialState.slice1);
        });
        it('should referenced sliced state normally', () => {
          expect(history2state(sixthState.normalState)).to.eql(sixthState.slice1);
          expect(sixthState.slice1).to.equal(initialState.slice1);
        });
        it('should work normally after referencing slices', () => {
          expect(history2state(seventhState.normalState).value).to.equal(
            history2state(sixthState.normalState).value - 1
          );
...
runTests('Initial History and Filter (Exclude Actions)', {
  ...
  initialStoreState: {
    past: Array(4).fill().map((v, i) => ({ value: i })),
    value: 4,
    future: Array(3).fill().map((v, i) => ({ value: i + 5 }))
  },
  testConfig: {
